...
def test_routes(client):
    ...
    assert client.get(flask.url_for('committee_sites.reporters')).status_code == 200
    assert client.get(flask.url_for('committee_sites.bylaws')).status_code == 200
    assert client.get(flask.url_for('committee_sites.ascit_bylaws')).status_code == 200
    assert client.get(flask.url_for('committee_sites.honor_system_handbook')).status_code == 200
