...
def test_text_editor_page(client):
    assert client.post(flask.url_for('editor.editor', input_text='TESTING TESTING', title="TEST")).status_code == 200
    client.post(flask.url_for('editor.save'))
    #assert flask.request.args['input_text'] == 'TESTING TESTING'
    #assert flask.request.args['title'] == 'TEST'
    assert client.get(flask.url_for('uploads.display', url=title)).status_code == 200
