...
def get_BoC_member():
    ...
    s = sqlalchemy.sql.select(["first_name, last_name, pos_name"]).select_from(sqlalchemy.text(sqlText1 + sqlText2))
    ...
    pos_house = []
    counter = 0
    for fname, lname, pos_name in result:
        name = fname + lname
        if pos_name == 'Chair' or 'ecretary' in pos_name:
            high_pos.append((name, pos_name))
        else:
            pos_house.append((name, pos_name))
